...
initial_conditions = {'rx': 1., 'ry': 0., 'rz': 0., 'Vx': 0., 'Vy': 0., 'Vz': 0., 'm': 15103./mscale}
#11414.
...
# Issue connections of mass constraint
# prob.model.add_subsystem('ConstraintMass', constraint_mass) ###
# prob.model.connect('integrator_group.state:m', 'ConstraintMass.m') ###
...
# prob.model.add_constraint('ConstraintMass.CM', lower=4000./mscale, indices=[-1]) # This works for min time
# prob.model.add_constraint('ConstraintMass.CM', lower=5000./mscale, indices=[-1]) # Max Prop

# prob.model.add_constraint('integrator_group.state:m', lower=1000./mscale, indices=[-1]) # Use for min time
# prob.model.add_constraint('integrator_group.state:m', lower=6000./mscale, indices=[-1]) # Use for max prop
prob.model.add_constraint('integrator_group.state:m', lower=1000./mscale, indices=[-1]) # Use for max prop

# prob.model.add_constraint('integrator_group.state:rx', lower=0.)
# prob.model.add_constraint('integrator_group.state:ry', lower=0.)
# prob.model.add_constraint('integrator_group.state:rz', lower=0.)
...
prob.driver.opt_settings['Major feasibility tolerance'] = 2e-9
prob.driver.opt_settings['Major optimality tolerance'] = 2e-9
...
# print("All data")
# print(data)
# pickle.dump(data,open("data_no_om.dat","wb"))
